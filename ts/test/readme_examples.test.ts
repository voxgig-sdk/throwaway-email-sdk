...
//     (\`new ThrowawayEmailSDK(...)\` / \`ThrowawayEmailSDK.test(...)\` become
//     \`ThrowawayEmailSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ThrowawayEmailSDK'
...
const TEST_SEED = {"entity":{"dns_query":{"test01":{"id":"test01"}},"domain":{"test01":{"id":"test01"}},"email":{"test01":{"id":"test01"}},"list":{"test01":{"id":"test01"}},"resolve":{"test01":{"id":"test01"}},"v2n":{"test01":{"id":"test01"}},"v3n":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ThrowawayEmailSDK(...)\` and any
// \`ThrowawayEmailSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ThrowawayEmailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ThrowawayEmailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
