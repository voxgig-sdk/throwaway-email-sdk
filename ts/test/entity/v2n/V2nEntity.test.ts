...
describe('V2nEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'v2n.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set THROWAWAY_EMAIL_TEST_V2N_ENTID JSON to run live')
      return
    }
    ...
    let v2n_ref01_data = Object.values(setup.data.existing.v2n)[0] as any

    // LOAD: skipped — no entity id field and load requires path params.
    // Entity-var is declared here so later flow steps still compile.
    const v2n_ref01_ent = client.V2n()
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/v2n/V2nTestData.json')
  ...
  const idmapEnvVal = process.env['THROWAWAY_EMAIL_TEST_V2N_ENTID']
  ...
  idmap = env['THROWAWAY_EMAIL_TEST_V2N_ENTID']
