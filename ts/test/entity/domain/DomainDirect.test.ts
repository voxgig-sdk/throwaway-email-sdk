...
describe('DomainDirect', async () => {
  ...
  test('direct-load-domain', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-domain', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['THROWAWAY_EMAIL_TEST_DOMAIN_ENTID']
