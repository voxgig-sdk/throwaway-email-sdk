...
describe('ListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set THROWAWAY_EMAIL_TEST_LIST_ENTID JSON to run live')
      return
    }
    ...
    const list_ref01_data_dt0 = await list_ref01_ent.load(list_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['THROWAWAY_EMAIL_TEST_LIST_ENTID']
  ...
  idmap = env['THROWAWAY_EMAIL_TEST_LIST_ENTID']
