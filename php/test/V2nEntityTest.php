...
class V2nEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ThrowawayEmailSDK::test(null, null);
        $ent = $testsdk->V2n(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = v2n_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "v2n." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set THROWAWAY_EMAIL_TEST_V2N_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function v2n_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/v2n/V2nTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["v2n01", "v2n02", "v2n03", "v201", "v202", "v203"] as $k) {
    ...
    $entid_env_raw = getenv("THROWAWAY_EMAIL_TEST_V2N_ENTID");
