...
// ThrowawayEmail SDK feature test
//
...
class FtCtrl extends ThrowawayEmailControl
{
...
class FtClient
{
    ...
    private ThrowawayEmailUtility $utility;

    public function __construct(ThrowawayEmailUtility $utility, array $options)
    {
    ...
    public function get_utility(): ThrowawayEmailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ThrowawayEmailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ThrowawayEmailContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ThrowawayEmailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ThrowawayEmailConfig::make_feature($name);
    ...
    private static function buildUrl(ThrowawayEmailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ThrowawayEmailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ThrowawayEmailResponse($fetched);
        }
    ...
    private function fail(ThrowawayEmailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ThrowawayEmailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
