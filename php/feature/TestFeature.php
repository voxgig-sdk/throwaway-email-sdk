...
// ThrowawayEmail SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ThrowawayEmailTestFeature extends ThrowawayEmailBaseFeature
{
    ...
    public function init(ThrowawayEmailContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ThrowawayEmailContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ThrowawayEmailParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ThrowawayEmailContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ThrowawayEmailContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ThrowawayEmailParam::call($ctx, $k);
