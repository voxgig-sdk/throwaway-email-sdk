...
// ThrowawayEmail SDK retry feature
...
class ThrowawayEmailRetryFeature extends ThrowawayEmailBaseFeature
{
    ...
    public function init(ThrowawayEmailContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ThrowawayEmailContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ThrowawayEmailContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ThrowawayEmailContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
