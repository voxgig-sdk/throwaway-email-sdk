...
class ThrowawayEmailMakeError
{
    public static function call(?ThrowawayEmailContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ThrowawayEmailContext([], null);
        }
        $op = $ctx->op ?? new ThrowawayEmailOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ThrowawayEmailResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ThrowawayEmailError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ThrowawayEmailError('', $msg, $ctx);
        ...
        if ($err instanceof ThrowawayEmailError) {
