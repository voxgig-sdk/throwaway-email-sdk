...
module ThrowawayEmailUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ThrowawayEmailResponse.new({})
    ...
    if fetch_err
      ...
      response = ThrowawayEmailResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ThrowawayEmailResponse.new(fetched)
    else
