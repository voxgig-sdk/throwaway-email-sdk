...
from throwawayemail_sdk.throwawayemail_types import (
    DnsQuery,
    DnsQueryLoadMatch,
    DnsQueryCreateData,
)
...
class DnsQueryEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DnsQueryEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> DnsQuery:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DnsQuery().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DnsQueryCreateData, ctrl=None) -> DnsQuery:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
