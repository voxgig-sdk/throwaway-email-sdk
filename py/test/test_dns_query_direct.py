...
class TestDnsQueryDirect:

    def test_should_direct_load_dns_query(self):
        setup = _dns_query_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-dns_query", "live" if setup["live"] else "unit")
        if _skip:
...
def _dns_query_direct_setup(mockres):
