...
class TestResolveDirect:
    ...
    def test_should_direct_load_resolve(self):
        setup = _resolve_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-resolve", "live" if setup["live"] else "unit")
        if _skip:
...
def _resolve_direct_setup(mockres):
