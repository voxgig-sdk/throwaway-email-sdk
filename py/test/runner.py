# ThrowawayEmail SDK test runner
...
class ThrowawayEmailTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ThrowawayEmailTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ThrowawayEmailTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ThrowawayEmailTestRunner.getenv("THROWAWAY_EMAIL_TEST_LIVE")
        override = ThrowawayEmailTestRunner.getenv("THROWAWAY_EMAIL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ThrowawayEmailTestRunner.getenv(key)
        ...
        explain = ThrowawayEmailTestRunner.getenv("THROWAWAY_EMAIL_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ThrowawayEmailTestRunner._test_control is not None:
            return ThrowawayEmailTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ThrowawayEmailTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ThrowawayEmailTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ThrowawayEmailTestRunner.load_test_control()
...
def load_env_local():
    ThrowawayEmailTestRunner.load_env_local()


def env_override(m):
    return ThrowawayEmailTestRunner.env_override(m)


def entity_data(v):
    return ThrowawayEmailTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ThrowawayEmailTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ThrowawayEmailTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ThrowawayEmailTestRunner.load_test_control()


def live_delay_ms():
    return ThrowawayEmailTestRunner.live_delay_ms()
