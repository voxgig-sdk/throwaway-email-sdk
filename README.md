...
API for detecting disposable/throwaway email domains with trait-based analysis

Learn more about throwaway.cloud API at [throwaway.cloud](https://throwaway.cloud).
...
const dnsquery = await client.DnsQuery().load({ dns: "example" })
```
...
| Go CLI | `github.com/voxgig-sdk/throwaway-email-sdk/go-cli` | `go install github.com/voxgig-sdk/throwaway-email-sdk/go-cli/cmd/throwaway-email@latest` |
| Go MCP server | `github.com/voxgig-sdk/throwaway-email-sdk/go-mcp` | `go get github.com/voxgig-sdk/throwaway-email-sdk/go-mcp@latest` |
...
// Load a specific v2n (returns a V2n)
...
dnsquery = client.DnsQuery().load({"dns": "example_dns"})
print(dnsquery)
```
...
v2n, err := client.V2n(nil).Load(
    map[string]any{"subject": "example_subject"}, nil,
)
if err != nil {
    panic(err)
}
...
local dnsquery, err = client:DnsQuery():load({ dns = "example_dns" })
print(dnsquery)
```
