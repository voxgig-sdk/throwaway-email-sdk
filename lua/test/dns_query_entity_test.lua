...
describe("DnsQueryEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set THROWAWAY_EMAIL_TEST_DNS_QUERY_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    dns_query_ref01_data = helpers.to_map(type(dns_query_ref01_data_result) == 'table' and dns_query_ref01_data_result.data_get and dns_query_ref01_data_result:data_get() or dns_query_ref01_data_result)
...
function dns_query_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("THROWAWAY_EMAIL_TEST_DNS_QUERY_ENTID")
