...
local DnsQueryEntity = {}
DnsQueryEntity.__index = DnsQueryEntity


function DnsQueryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DnsQueryEntity)
  self._name = "dns_query"
  self._client = client
...
function DnsQueryEntity:get_name()
  return self._name
end


function DnsQueryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DnsQueryEntity.new(self._client, opts)
end
...
function DnsQueryEntity:mark_deleted()
  self._deleted = true
end


function DnsQueryEntity:deleted()
  return true == self._deleted
end


function DnsQueryEntity:data_set(args)
  if args ~= nil then
...
function DnsQueryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DnsQueryEntity:match_set(args)
  if args ~= nil then
...
function DnsQueryEntity:match_get()
...
function DnsQueryEntity:stream(action, args, callopts)
...
function DnsQueryEntity:_run_op(ctx, post_done)
  local utility = self._utility
