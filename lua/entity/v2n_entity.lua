...
local V2nEntity = {}
V2nEntity.__index = V2nEntity


function V2nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V2nEntity)
  self._name = "v2n"
  self._client = client
...
function V2nEntity:get_name()
  return self._name
end


function V2nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V2nEntity.new(self._client, opts)
end
...
function V2nEntity:mark_deleted()
  self._deleted = true
end


function V2nEntity:deleted()
  return true == self._deleted
end


function V2nEntity:data_set(args)
  if args ~= nil then
...
function V2nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V2nEntity:match_set(args)
  if args ~= nil then
...
function V2nEntity:stream(action, args, callopts)
...
---@param reqmatch V2nLoadMatch
---@param ctrl? table
---@return V2n
---@return string? err
function V2nEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function V2nEntity:_run_op(ctx, post_done)
  local utility = self._utility
