...
local V3nEntity = {}
V3nEntity.__index = V3nEntity


function V3nEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, V3nEntity)
  self._name = "v3n"
  self._client = client
...
function V3nEntity:get_name()
  return self._name
end


function V3nEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return V3nEntity.new(self._client, opts)
end
...
function V3nEntity:mark_deleted()
  self._deleted = true
end


function V3nEntity:deleted()
  return true == self._deleted
end


function V3nEntity:data_set(args)
  if args ~= nil then
...
function V3nEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function V3nEntity:match_set(args)
  if args ~= nil then
...
function V3nEntity:stream(action, args, callopts)
...
---@param reqmatch V3nLoadMatch
---@param ctrl? table
---@return V3n
---@return string? err
function V3nEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function V3nEntity:_run_op(ctx, post_done)
  local utility = self._utility
