...
local ResolveEntity = {}
ResolveEntity.__index = ResolveEntity


function ResolveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResolveEntity)
  self._name = "resolve"
  self._client = client
...
function ResolveEntity:get_name()
  return self._name
end


function ResolveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResolveEntity.new(self._client, opts)
end
...
function ResolveEntity:mark_deleted()
  self._deleted = true
end


function ResolveEntity:deleted()
  return true == self._deleted
end


function ResolveEntity:data_set(args)
  if args ~= nil then
...
function ResolveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResolveEntity:match_set(args)
  if args ~= nil then
...
function ResolveEntity:match_get()
...
function ResolveEntity:stream(action, args, callopts)
...
---@param reqmatch ResolveLoadMatch
---@param ctrl? table
---@return Resolve
---@return string? err
function ResolveEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ResolveEntity:_run_op(ctx, post_done)
  local utility = self._utility
