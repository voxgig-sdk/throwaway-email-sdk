# ThrowawayEmail Lua SDK

The Lua SDK for the ThrowawayEmail API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:DnsQuery(nil):load({ id = "example_id" }, nil)
...
```lua
-- Create
local created, _ = client:DnsQuery(nil):create({ name = "Example" }, nil)
...
local result, err = client:ThrowawayEmail(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── throwaway-email_sdk.lua    -- Main SDK module
...
The main module (`throwaway-email_sdk`) exports the SDK constructor
