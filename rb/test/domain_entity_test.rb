...
class DomainEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set THROWAWAY_EMAIL_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["THROWAWAY_EMAIL_TEST_DOMAIN_ENTID"]
