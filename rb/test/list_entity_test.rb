...
class ListEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = ThrowawayEmailSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set THROWAWAY_EMAIL_TEST_LIST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    list_ref01_data_dt0_loaded = list_ref01_ent.load(list_ref01_match_dt0, nil)
    assert !list_ref01_data_dt0_loaded.nil?
...
def list_basic_setup(extra)
  ...
  entid_env_raw = ENV["THROWAWAY_EMAIL_TEST_LIST_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "THROWAWAY_EMAIL_TEST_LIST_ENTID" => idmap,
    "THROWAWAY_EMAIL_TEST_LIVE" => "FALSE",
    "THROWAWAY_EMAIL_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["THROWAWAY_EMAIL_TEST_LIST_ENTID"])
