...
class ResolveEntity
  ...
  def make
    opts = @_entopts.dup
    ResolveEntity.new(@_client, opts)
  end
  ...
  # @return [Resolve, Hash] the current Resolve data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Resolve fields)
  def match_get
  ...
  #
  # @param reqmatch [ResolveLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Resolve.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Resolve, Hash] the loaded Resolve; raises ThrowawayEmailError on failure
