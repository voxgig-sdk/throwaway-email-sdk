# ThrowawayEmail SDK DnsQuery entity
...
class DnsQueryEntity
  ...
  def make
    opts = @_entopts.dup
    DnsQueryEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ThrowawayEmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DnsQuery, Hash] the current DnsQuery data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ThrowawayEmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DnsQuery fields)
  def match_get
  ...
  #
  # @param reqmatch [DnsQueryLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.DnsQuery.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [DnsQuery, Hash] the loaded DnsQuery; raises ThrowawayEmailError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ThrowawayEmailHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqdata [DnsQueryCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [DnsQuery, Hash] the created DnsQuery; raises ThrowawayEmailError on failure
