...
require_relative 'ThrowawayEmail_types'
...
class ThrowawayEmailSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ThrowawayEmailHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ThrowawayEmailHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ThrowawayEmailFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ThrowawayEmailUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ThrowawayEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ThrowawayEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ThrowawayEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ThrowawayEmailError.new(
        "#{op}_allow",
        "ThrowawayEmailSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ThrowawayEmailError => err
    ...
    if fetched.is_a?(Hash)
      status = ThrowawayEmailHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ThrowawayEmailError.new(
        "graphql_error", "ThrowawayEmailSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ThrowawayEmailSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
