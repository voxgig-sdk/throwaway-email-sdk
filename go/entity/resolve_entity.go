...
func NewResolveEntity(client *core.ThrowawayEmailSDK, entopts map[string]any) *ResolveEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ResolveEntity) GetName() string { return e.name }
...
func (e *ResolveEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ResolveEntity) Deleted() bool {
	return e.deleted
}


func (e *ResolveEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewResolveEntity(e.client, opts)
}

func (e *ResolveEntity) Data(args ...any) any {
...
func (e *ResolveEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Resolve; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ResolveEntity) DataTyped(data ...Resolve) Resolve {
	if len(data) > 0 {
		return typedFrom[Resolve](e.Data(asMap(data[0])))
	}
	return typedFrom[Resolve](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Resolve (all fields
// optional at the wire level).
func (e *ResolveEntity) MatchTyped(match ...Resolve) Resolve {
	if len(match) > 0 {
		return typedFrom[Resolve](e.Match(asMap(match[0])))
	}
	return typedFrom[Resolve](e.Match())
}
...
func (e *ResolveEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ResolveEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ResolveLoadMatch and returns an Resolve. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *ResolveEntity) LoadTyped(reqmatch ResolveLoadMatch, ctrl map[string]any) (Resolve, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Resolve{}, err
	}
	return typedFrom[Resolve](res), nil
}



func (e *ResolveEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *ResolveEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *ResolveEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *ResolveEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *ResolveEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
