...
func TestEmailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set THROWAWAYEMAIL_TEST_EMAIL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func emailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("THROWAWAYEMAIL_TEST_EMAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["THROWAWAYEMAIL_TEST_EMAIL_ENTID"])
