...
func TestV3nDirect(t *testing.T) {
...
func v3nDirectSetup(mockres any) *v3nDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"THROWAWAY_EMAIL_TEST_V3N_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["THROWAWAY_EMAIL_TEST_V3N_ENTID"]; ok {
		...
		return &v3nDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &v3nDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
