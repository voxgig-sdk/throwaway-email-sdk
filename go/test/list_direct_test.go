...
func TestListDirect(t *testing.T) {
	...
	t.Run("direct-load-list", func(t *testing.T) {
		setup := listDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-list", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func listDirectSetup(mockres any) *listDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"THROWAWAY_EMAIL_TEST_LIST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["THROWAWAY_EMAIL_TEST_LIST_ENTID"]; ok {
