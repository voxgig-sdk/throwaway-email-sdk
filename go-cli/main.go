// Command ThrowawayEmail-cli is a CLI + REPL for the ThrowawayEmail SDK,
...
const prompt = "throwaway-email"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dns_query domain email list resolve v2n v3n"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      THROWAWAY_EMAIL_APIKEY (api key), THROWAWAY_EMAIL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: THROWAWAY_EMAIL_APIKEY carries the API key and
	// THROWAWAY_EMAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("THROWAWAY_EMAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("THROWAWAY_EMAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewThrowawayEmailSDK(opts)
