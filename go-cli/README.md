...
boru-driven command-line client **and** interactive REPL for the ThrowawayEmail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/throwaway-email-cli)
make build

# 2. See usage (words, entities, env vars)
./throwaway-email-cli --help

# 3. Provide credentials once, via the environment
export THROWAWAY_EMAIL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./throwaway-email-cli load 1 dns_query            # {id:1} shorthand
./throwaway-email-cli load '{id:1}' dns_query       # explicit match map

# 5. Override the API base URL for a single call
THROWAWAY_EMAIL_BASE=https://api.example.com ./throwaway-email-cli load 1 dns_query

# 6. No arguments -> interactive REPL
./throwaway-email-cli
throwaway-email> load 1 dns_query
throwaway-email> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/throwaway-email-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/throwaway-email-cli load 1 dns_query
   ```
...
```sh
./throwaway-email-cli load 1 dns_query          # scalar shorthand for {id:1}
./throwaway-email-cli load '{id:1}' dns_query     # explicit match map
```
...
./throwaway-email-cli load 1 dns_query
```
...
Run with no arguments to open a REPL (prompt `throwaway-email>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dns_query`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/throwaway-email-cli`. |
...
(`./throwaway-email-cli <expr>`) and interactively (the REPL), and expressions compose the
